...
`
var varBanner = `bannerPlaseholder${i}`
...
item.addEventListener("click", function() {
  $(item).closest(".window").addClass("hidden")
})
});
...
let banners = document.getElementsByClassName("window")
for (var i = 0; i < banners.length; i++) {
  banners[i].classList.remove("hidden")
  await timer(8000)
}
}
setTimeout(showBannerModale, 10000);

async function showBannerSmall() {
let banners = document.getElementsByClassName("bannersmalldrag")
for (var i = 0; i < banners.length; i++) {
  await timer(4500)
  banners[i].classList.remove("hidden")
...
setTimeout(showBannerSmall, 10000);
